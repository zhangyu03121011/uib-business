...
<script type="text/javascript">
	...
	var $specificationGroupIds;
	var tables = ["base_info_tab","product_introduce_tab","product_image_tab","product_parameter_tab","product_property_tab","product_pecification_tab","product_price_tab","sales_area_tab"];
		...
		$('#product_image_tab').delegate("tr td input[type=\"button\"]","click",function() {
		...
		$(function(){
		    $("#tab_header").children("li").click(function(){
				...
				for(var i = 0 ; i < tables.length ; i++){
					...
					if(tables[i] == table_id){
						if((i==4||i==5)&&$("#productCategoryId").val().length<1){
							$("#product_property_tab","#product_pecification_tab").hide();
						}else{
							var $tab = $("#"+table_id).attr("style","display:table;width:100%");
						}
					}
		    ...
		    $.ajax({
				...
				success: function(jsonData){
					...
					$.each(jsonData,function(index,value){
						...
						tdHtml += "</select></td>";
						$("#specificationGroupSelect :checkbox").each(function(index){
			...
			$("#specificationProductTable").on("change","select",function(){
				var groupId = $(this).attr("name").substring(7);
				//console.info("groupId:"+groupId);
				$specificationProductTable.find("input[name^='specificationGroup_"+groupId+"']").each(function(){
					var $this = $(this);
					...
					console.info("groupId:"+groupId+",ids:"+$this.val());
				});
			});
			//分类变化
		...
		function alter_specification_tab(productCategoryId){
			//var groups =$("#specificationProductTable").data("specificationGroupsJson");
			//if(typeof(groups)=='undefined'||groups==null){
			//	groups = $.data(document,"specificationGroupsJson",${fns:toJson(specificationGroups)});
			//}
			 $.ajax({
		...
		function propertyChange(value){
			$.ajax({
				url: "../../product/product_property/list?gourpId="+value,
				type: "GET",
				dataType: "json",
				success: function(data) {
					var optHtml = "";
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/product/product/form?id=${product.id}">商品<shiro:hasPermission name="product:product:edit">${not empty product.id?'修改':'添加'}</shiro:hasPermission>
	...
	<form:form id="inputForm" modelAttribute="product" action="${ctx}/product/product/save" method="post" class="form-horizontal">
		...
		<div id="base_info_tab">
			...
			<div class="control-group">
				...
				<div class="controls">
					<form:input path="fullName" htmlEscape="false" maxlength="128" class="input-xlarge required" />
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
			<div class="control-group">
				...
				<div class="controls">
					<form:input path="productNo" htmlEscape="false" class="input-xlarge" />
				</div>
			</div>
			<%--<div class="control-group">
				<label class="control-label">销售价格：</label>
				<div class="controls">
					<form:input path="price" htmlEscape="false" class="input-xlarge number required" /><font color="red">*</font>
				</div>
			</div> --%>
			<%-- <div class="control-group">
				<label class="control-label">是否会员价：</label>
				<div class="controls">
					<form:checkbox path="isMemberPrice" value="1" eclass="input-xlarge " />
				</div>
			</div> --%>
			<div class="control-group">
				<label class="control-label">成本价：</label>
				<div class="controls">
					<form:input path="cost" htmlEscape="false" class="input-xlarge  number required" />
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
			<div class="control-group">
				...
				<div class="controls">
					<form:input path="marketPrice" htmlEscape="false" class="input-xlarge  number required" /><span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
			<div class="control-group">
				...
				<div class="controls">
					...
					<input type="button" id="image_select_button" value="选择文件" />
			...
			<div class="control-group">
				...
				<div class="controls">
					<form:checkbox path="hotSell" htmlEscape="false" maxlength="1" value="1" class="input-xlarge " />
					热销
				</div>
				<div class="controls">
					<form:checkbox path="newest" htmlEscape="false" maxlength="1" value="1" class="input-xlarge " />
					最新
				</div>
				<div class="controls">
					<form:checkbox path="promotion" htmlEscape="false" maxlength="1" value="1" class="input-xlarge " />
					活动促销
				</div>
			</div>
			<!--  <div class="control-group">
		...
		-->
			...
			<div class="control-group">
				...
				<div class="controls">
					<form:input path="stock" htmlEscape="false" maxlength="11" class="input-xlarge  digits number required" /><span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
		<div class="control-group">
		...
		<table id="product_introduce_tab" style="display: none; width: 100%;">
			<tr>
				<td>
					<form:textarea id="editor" path="introduction"
						class="editor" style="width: 100%;" htmlEscape="false" />
				</td>
<%-- 				<td><textarea id="editor" name="introduction"  value="${product.introduction}" class="editor" style="width: 100%;"></textarea></td> --%>
			</tr>
		</table>
		<!-- 商品图片 -->
		<table id="product_image_tab" width="100%" style="display: none;">
			<thead>
				<tr>
					<td colspan="4"><a href="javascript:;" id="addProductImage" class="button">添加图片</a></td>
				</tr>
			</thead>
			<tr class="title">
				<td>文件</td>
		...
		<div id="product_parameter_tab" style="display: none; width: 100%;">
			<c:forEach items="${parameterGroups }" var="parameterGroup" varStatus="groupStatus">
				<div value="${parameterGroup.productCategoryId}" style="display: none;">
					<div class="control-group">
						<label style="font-weight: bold;" class="control-label">${parameterGroup.name}:</label>
					</div>
					<div>
						<c:forEach items="${parameterGroup.productParameterList }" var="productParameter" varStatus="status">
							<div class="control-group">
								<label class="control-label">${productParameter.name}</label>
								<div class="controls">
									<form:input path="" name="parameter_${productParameter.id}" value="${product.parameterValue[productParameter]}" htmlEscape="false" maxlength="32" class="input-xlarge " />
								</div>
							</div>
						</c:forEach>
					</div>
		...
		<div id="product_property_tab" style="display: none; width: 100%;">
			<c:forEach items="${propertyGrop }" var="item">
				<div class="control-group">
					...
					<div class="controls">
						<form:select path="" name="property_${item.id}" class="input-xlarge">
							<form:options items="${item.productPropertyList}" itemLabel="name" itemValue="id" htmlEscape="false" />
						</form:select>
					</div>
				</div>
			</c:forEach>
		...
		<div id="product_pecification_tab" style="display: none; width: 100%;">
			<table style="width: 100%;" id="specificationGroupsTable">
				...
				<tr>
					<td>
						<div id="specificationGroupSelect" class="specificationGroupSelect">
							<ul>
								<c:forEach items="${specificationGroups}" var="specificationGroup">
									<li><label> <input type="checkbox" name="specificationGroupIds" value="${specificationGroup.id}" />${specificationGroup.name} <c:if
			...
			<table id="specificationProductTable" style="width: 100%">
				<tr>
					<td width="60">&nbsp;</td>
					<c:forEach items="${specificationGroups}" var="specificationGroup">
						<td class="specification_${specificationGroup.id} hidden" style="display: none;">${specificationGroup.name} <c:if
								test="${specificationGroup.remarks!=null and specificationGroup.remarks!=''}">
								<span class="gray">[${specificationGroup.remarks}]</span>
							</c:if>
						</td>
				...
				<!--<tr>
							...
							<c:forEach items="${specificationGroups}" var="specificationGroup" varStatus="status">
								<td>
									<select name="productSpecificationIds" class="input-xlarge">
										<c:forEach items="${specificationGroup.productSpecificationList}" var="productSpecification">
											<option value="${productSpecification.id}">${productSpecification.name }</option>
										</c:forEach>
									</select>
								</td>
							</c:forEach>
							<td>
								<a href="javascript:;" id="deleteSpecificationProduct">删除</a>
							</td>
						</tr>-->
			</table>					
		</div>
		<!-- 商品价格 -->
		<table id="product_price_tab" style="display: none; width: 100%;">
			<tr class="title">
				<td>会员等级</td>
				<td>售价（元）</td>
				<td>B端供货价（元）</td>
			...
			<tr>
				<td><input type="text" value="${memRank.name}" style="width:80px;" disabled="disabled"/></td>
<%-- 				<td><input type="text" name="productPriceRefList[${status.index}].sellPrice"  value="${productPriceRefList[${status.index}].sellPrice}"/></td> --%>
<%-- 				<td><input type="text" name="productPriceRefList[${status.index}].bSupplyPrice" value="${productPriceRefList.bSupplyPrice}" /></td> --%>
				<td><form:input path="productPriceRefList[${status.index}].sellPrice" htmlEscape="false" class="input-xlarge" /></td>
				<td><form:input path="productPriceRefList[${status.index}].bSupplyPrice" htmlEscape="false" class="input-xlarge" /></td>
		...
		<%-- <div class="control-group">
			<label class="control-label">市场价：</label>
			<div class="controls">
				<form:input path="marketPrice" htmlEscape="false" class="input-xlarge  number required" /><font color="red">*</font>
			</div>
		</div> --%>
		...
		<table id="treeTable" class="table table-striped table-bordered table-condensed">
			<thead><tr><th><th>区域名称</th></tr></thead>
	...
	<div>
		<input type="hidden" id="hidden_value" value="${specificationGroupsString}" />
	</div>
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.code}}" pId="{{pid}}">
			<td><input id="areaId_{{row.code}}" class="areaId" name="areaId" type="checkbox" value="{{row.id}}{{row.type}}"  onclick="checkAll('{{row.parentIds}}',{{row.code}},this,{{row.type}})">
