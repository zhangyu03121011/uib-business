...
<body>
	...
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/product/special/updateFormView?id=${product.specialId}&updateFlag=${product.updateFlag}">专题基本信息</a></li>
		<li class="active"><a href="${ctx}/product/special/updateSelectProduct?specialId=${product.specialId}&updateFlag=${product.updateFlag}">专题商品查看</a></li>
		<c:choose>
			<c:when test="${product.specialId!=null && not empty product.specialId}">
			<li><a href="${ctx}/product/special/updateProductList?specialId=${product.specialId}&updateFlag=${product.updateFlag}">专题商品添加</a></li>
			</c:when>
			<c:otherwise>
		...
		<c:forEach items="${page.list}" var="specialRef">
			<tr>
				...
				<shiro:hasPermission name="product:special:edit"><td>
					<a href="${ctx}/product/special/updateDeleteSpecialProduct?id=${specialRef.id}&&specialId=${specialRef.specialId}" onclick="return confirmx('确认要删除该专题商品吗？', this.href)">删除</a>
