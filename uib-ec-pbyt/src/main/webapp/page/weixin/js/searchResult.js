...
function getProduct(url, divshow){
	...
	AJAX.call( url, "post", "json", "", true, function(result){
		...
		$("li").remove(".scrimg");
		isApend = true;
		$("#searchkey").val(result.data.productName);
...
$(document).ready(function(){
	var productCategoryName = URL_PARAM.getParam("productCategoryName");
	$(".tab-bar-item").click(function(){
		...
		if(curIndex == 1 && (curIndex == lastIndex || $("#firstPriceSelect").val() == "true")){
			...
			getProduct(rootPath + "/wechat/product/findByOrderAndPage?orderparam=" + orderparam + "&page=1&down="+down+"&productCategoryName="+productCategoryName,$("#price-ul"));
		}
		...
		if(curIndex == 2 && (curIndex == lastIndex || $("#firstSalesSelect").val() == "true")){
			...
			getProduct(rootPath + "/wechat/product/findByOrderAndPage?orderparam=" + orderparam + "&page=1&down="+down+"&productCategoryName="+productCategoryName,$("#sales-ul"));
		}
	...
	var orderparam="date";
	//加载最新
	//getProduct(rootPath + "/wechat/product/findByOrderAndPage?orderparam=" + orderparam + "&page=1&down=true",$("#all-product-ul"));
	getProduct(rootPath + "/wechat/product/findByOrderAndPage?orderparam=" + orderparam + "&page=1&down=true"+"&productCategoryName="+productCategoryName,$("#all-product-ul"));
	...
	$(document).on("scrollstart",function(){
		if(isTop){
			...
			if($("#menu-index").val() == 0 && notEnd0){
				...
				orderparam="date";
				//加载最新
				getProduct(rootPath + "/wechat/product/findByOrderAndPage?orderparam=" + orderparam + "&page="+page+"&down="+down+"&productCategoryName="+productCategoryName,$("#all-product-ul"));
			}
	      	if($("#menu-index").val() == 1 && notEnd1){
	  			...
	  			orderparam="price";
	  			//加载价格
	  			getProduct(rootPath + "/wechat/product/findByOrderAndPage?orderparam=" + orderparam + "&page="+page+"&down="+down+"&productCategoryName="+productCategoryName,$("#price-ul"));
	      	}
	      	if($("#menu-index").val() == 2 && notEnd2){
	  			...
	  			orderparam="sales";
	  			//加载销量
	  			getProduct(rootPath + "/wechat/product/findByOrderAndPage?orderparam=" + orderparam + "&page="+page+"&down="+down+"&productCategoryName="+productCategoryName,$("#sales-ul"));
	      	}
	      //判断这个标签下面是否有这个div
