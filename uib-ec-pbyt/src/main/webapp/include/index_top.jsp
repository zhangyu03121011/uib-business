	...
	$(function(){
		$.ajax({
			...
			success: function(cart){
				...
				if(cart!=null&&cart.cartItems!=null){
					...
					$.each(cart.cartItems,function(indec,cartItem){
						cart_item_html += "<dt><a href=\"#\"><img src=\"${UPLOAD_IMAGE_PATH}"+(cartItem.product.image==null?"123":cartItem.product.image)+"\" /></a>"+
						  "<p>"+
						  "<a class=\"tit\" href=\"${pageContext.request.contextPath  }/f/product/details?productId="+(cartItem.product.id)+"\">"+ (cartItem.product.name)+
			              "</a> <span>¥"+(cartItem.product.price)+"x"+(cartItem.quantity)+"</span> <a class=\"del\" href=\"javascript:void(0);\" cartItemId=\""+cartItem.id+"\" data-id=\""+cartItem.product.id+"\">删除</a>"+
	    				  "</p>"+
	  					  "</dt>";
					});
				}else{
				...
				cart_item_html += "<dd><em>购物车里还有"+(cart!=null?cart.quantity:0)+"件商品</em> <a href=\"${pageContext.request.contextPath  }/f/cart/list\">去购物车并结算</a></dd>";
				cart_item_html += "</dl>";
        ...
        <div class="tool_link">
				<c:choose>
						<c:when test="${not empty(member)}">
							${member.username},欢迎光临初云汇商城！   <a href="${pageContext.request.contextPath  }/f/logout">退出</a>
						</c:when>
						<c:otherwise>
							欢迎光临初云汇商城！&nbsp;<a href="${pageContext.request.contextPath  }/f/login">登录</a>&nbsp;或&nbsp;<a href="${pageContext.request.contextPath  }/f/reg/registerView">免费注册</a>
						</c:otherwise>
				</c:choose>
        </div>
			<ul class="quick-menu">
            <li class="user menu-item">
				  <div class="menu"><span class="menu-hd"><s></s>我的账户<b></b></span> 
					  <div class="menu-bd">
              ...
              <li class="cart menu-item">
				  <div class="menu"><span class="menu-hd"><s></s><a href="${pageContext.request.contextPath  }/f/cart/list">购物车(<font id="font_of_cart_count">0</font>)</a><b></b></span> 
...
<div class="life_logo"><img src="${pageContext.request.contextPath  }/static/images/logo.jpg"/></div>
...
<div class="life_searchhot">热门搜索：<a href="#">图书</a> <a href="#">美素</a> <a href="#">美赞臣</a> <a href="#">衬衫</a></div>
</div>
</div>
</div>
<!-- 搜索栏结束 -->
...
<div class="navsort w1200">
	<div class='allsort'>
		<div class='mt'><s></s><font>全部分类</font></div>
		<div class='mcindex'>
			      ...
			      <c:forEach items="${categoryList }" var="item" varStatus="idx">
			      		<c:choose>
			      				<c:when test="${idx.index == 0 }">
			      						<div class='item fore'>
												<span><h3><a href="${pageContext.request.contextPath }/f/product/list?productCategoryId=${item.id}">${item.text }</a></h3><s></s></span>
												<div class='i-mc'>
													...
													<div class='subitem'>
														<c:if test="${not empty(item.children)}">
															  <c:forEach items="${item.children }" var="childrenItem">
															  		<dl class='fore'>
																		<dt>
																			<a href='${pageContext.request.contextPath }/f/product/list?productCategoryId=${childrenItem.id}'>
																				${childrenItem.text}
																			</a>
																		</dt>
																		<c:if test="${not empty(childrenItem.children)}">
																			<dd>
																				<c:forEach items="${childrenItem.children }" var="ccItem">
																					<em><a href='${pageContext.request.contextPath }/f/product/list?productCategoryId=${ccItem.id}'>${ccItem.text}</a></em>
																				</c:forEach>
																			</dd>
																		</c:if>
																	</dl>
