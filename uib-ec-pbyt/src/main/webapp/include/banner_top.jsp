	...
	<div class="site-nav w1200">
		<div class="tool_link">
			<c:choose>
				<c:when test="${not empty(member)}">
							${member.username},欢迎光初云汇商城！   <a href="${pageContext.request.contextPath  }/f/logout">退出</a>
				</c:when>
				<c:otherwise>
							欢迎光初云汇商城！&nbsp;<a
		...
		<ul class="quick-menu">
			...
			<li class="cart menu-item">
				<div class="menu">
					<span class="menu-hd"><s></s>购物车(<font id="font_of_cart_count">0</font>)<b></b></span>
...
<div class="full-screen-slider">
	<ul class="slides">
		<c:if test="${not empty(makeup_banner_List)}">
			<c:forEach var="item" items="${makeup_banner_List}">
				<li style="background:url('${UPLOAD_IMAGE_PATH}${item.path}') no-repeat center top"><a href="${item.url }" target="_blank"></a></li>
			</c:forEach>
		</c:if>
	</ul>
</div>